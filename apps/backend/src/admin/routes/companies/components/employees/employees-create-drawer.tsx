...
export function EmployeeCreateDrawer({ company }: { company: QueryCompany }) {
  ...
  const handleSubmit = async (
    ...
    if (!employee) {
      toast.error("No se pudo vincular el usuario a la empresa");
      return;
    }

    setOpen(false);
    toast.success(
      `Usuario ${customer?.first_name} ${customer?.last_name} creado`
    );
  };
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      ...
      <Drawer.Content>
        <Drawer.Header>
          <Drawer.Title>Anadir usuario a la empresa</Drawer.Title>
        </Drawer.Header>
        <EmployeesCreateForm
