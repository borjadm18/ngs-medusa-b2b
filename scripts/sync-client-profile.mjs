...
const generatedBackendProductCatalogRegistryPath = path.join(
  generatedBackendProfileDir,
  "product-catalog-registry.ts"
)
...
const readProductCatalogCsv = (filePath) => {
  const text = fs.readFileSync(filePath, "utf8").trim()
  ...
  return lines
    ...
    .map((line, index) => {
      ...
      if (!record.handle || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(record.handle)) {
        throw new Error(`handle must be kebab-case on product CSV line ${lineNumber}`)
      }

      if (!record.title || !record.category || !record.sku) {
        throw new Error(
          `title, category and sku are required on product CSV line ${lineNumber}`
        )
      }

      const price = Number(record.price_eur)

      if (!Number.isFinite(price) || price <= 0) {
        throw new Error(`price_eur must be positive on product CSV line ${lineNumber}`)
      }
      ...
      return {
        ...
        sku: record.sku,
        variant_title: record.variant_title || record.color || "Default",
...
const backendProductCatalogImportLines = []
const backendProductCatalogRegistryEntries = []
...
for (const profileId of profileIds) {
  ...
  const sourceProductCatalogPath = path.join(
    profilesDir,
    profileId,
    "product-catalog.csv"
  )
  ...
  if (fs.existsSync(sourceProductCatalogPath)) {
    const productCatalog = readProductCatalogCsv(sourceProductCatalogPath)
    const targetProductCatalogPath = path.join(
      generatedBackendProfileDir,
      `${profileId}-product-catalog.json`
    )

    assertInside(targetProductCatalogPath, generatedBackendProfileDir)
    writeJson(targetProductCatalogPath, productCatalog)
    backendProductCatalogImportLines.push(
      `import ${identifier}ProductCatalog from "./${profileId}-product-catalog.json"`
    )
    backendProductCatalogRegistryEntries.push(
      `  "${profileId}": ${identifier}ProductCatalog,`
    )
    console.log(`Validated ${profileId} product catalog (${productCatalog.length})`)
  }
...
const generatedBackendProductCatalogSource = `// Generated by scripts/sync-client-profile.mjs. Do not edit manually.\n${backendProductCatalogImportLines.join(
  "\n"
)}\n\nexport const generatedProductCatalogProfiles = {\n${backendProductCatalogRegistryEntries.join(
  "\n"
)}\n}\n`

fs.writeFileSync(
  generatedBackendProductCatalogRegistryPath,
  generatedBackendProductCatalogSource,
  "utf8"
)
console.log(
  `Generated ${path.relative(rootDir, generatedBackendProductCatalogRegistryPath)}`
)
